...
if (typeof token === 'string' && token.length === 39) var room = HBInit({ roomName, maxPlayers, token, public: roomPublic, noPlayer: true })
else var room = HBInit({ roomName, maxPlayers, public: roomPublic, noPlayer: true })
...
const statsColor = 0xBEBEBE
const defaultColor = null

/* AUXILIARY */

let checkTimeVariable = false
let checkDrawWarning = false
let checkStadiumVariables = true
let unpauseTimeout
...
function calculateStadiumVariables () {
  if (checkStadiumVariables && teamR.length + teamB.length !== 0) {
    ...
    setTimeout(() => {
      ...
      const playerDisc = room.getPlayerDiscProperties(teamR.concat(teamB)[0].id)
...
function helpCommand (player, message) {
  ...
  if (msgArray.length === 0) {
    let commandString = '[PV] Player commands :'
    for (const [key, value] of Object.entries(commands)) {
...
function updateAdmins (excludedPlayerID = 0) {
  if (players.length !== 0 && players.filter((p) => p.admin).length < maxAdmins) {
    const playerArray = players.filter((p) => p.id !== excludedPlayerID && p.admin === false)
    const arrayID = playerArray.map((player) => player.id)
